...
# img_path = 'cat.jpg'
	...
	if not bbox:
		...
		region = image.img_to_array(region)
    		...
    		np.save(file=storage_dir+'processed_b/'+number, arr=reg_output)
		np.save(file=storage_dir+'processed_flatten_b/'+number, arr=reg_output2)
		np.save(file=storage_dir+'processed_predictions_b/'+number, arr=reg_output3)
